...
def construct_bb():
    ...
    for key in end_ins_dict:
        ...
        vertices[key] = block
        #k= vertices[key].get_instructions()
        #print("----------------")
        #log.info(k)
        edges[key] = []
...
def sym_exec_block(params):
    ...
    for instr in block_ins:
        ...
        params.instr = instr
        #log.info(params.stack)
        #log.info(params.instr)
        sym_exec_ins(params)
    if global_params.DEBUG_MODE:
        print("")
...
def sym_exec_ins(params):
    ...
    if instr_parts[0] == "STOP":
        ...
        if len(stack) > 1:
            ...
            if isReal(first) and isSymbolic(second):
                ...
                second = BitVecVal(second, 256)
            computed = first * second & UNSIGNED_BOUND_NUMBER #2 ** 256-1
            computed = simplify(computed) if is_expr(computed) else computed
            instruction_object.data_out = [computed]
            stack.insert(0, computed)
        ...
        if len(stack) > 1:
            ...
            raise ValueError('STACK underflow')
    elif instr_parts[0] == "ADDMOD": #(x+y)%m
        if len(stack) > 2:
            global_state["pc"] = global_state["pc"] + 1
            instruction_object = InstructionObject(instr_parts[0], [], [])
            first = stack.pop(0)
            ...
            raise ValueError('STACK underflow')
    elif instr_parts[0] == "MULMOD": #(x*y)%m
        if len(stack) > 2:
            global_state["pc"] = global_state["pc"] + 1
            instruction_object = InstructionObject(instr_parts[0], [], [])
            first = stack.pop(0)
    ...
    #
    elif instr_parts[0] == "LT": #less than
        if len(stack) > 1:
            global_state["pc"] = global_state["pc"] + 1
            instruction_object = InstructionObject(instr_parts[0], [], [])
            first = stack.pop(0)
            ...
            raise ValueError('STACK underflow')
    elif instr_parts[0] == "GT": #greater than
        if len(stack) > 1:
            global_state["pc"] = global_state["pc"] + 1
            instruction_object = InstructionObject(instr_parts[0], [], [])
            first = stack.pop(0)
    ...
    #
        ...
        if len(stack) > 0:
            global_state["pc"] = global_state["pc"] + 1
            #print("calldataload stack content:")
            #print(stack)
            ...
            if source_map:
                ...
                if source_code.startswith("function") and isReal(position):
                    ...
                    para_list=params_list
                    for para in para_list:
                        if para.find("bytes ")==0:
                            para=para.split(" ")[-1]
                            if para not in tp_list:
                                tp_list.append(para)
                                if_static.append(1)
                            index=index+1
                        elif para.find("string ")==0:
                            para=para.split(" ")[-1]
                            if para not in tp_list:
                                tp_list.append(para)
                                if_static.append(1)
                    ...
                    '''print("src:="+source_code)
                    print("Debug:new_var_name1="+new_var_name)'''
                    source_map.var_names.append(new_var_name)
                elif source_code.startswith("contract"):
                    idx1 = source_code.index(" ")+1
                    idx2 = source_code.find(" ",9,32)
                    contract_name = source_code[idx1:idx2]
                    new_var_name = "Id_" + contract_name
                elif source_code.startswith("function"):
                    indx=position
                    temp=str(indx)
                    para_num=temp[4: ]
                    para_num=para_num+"_num"
                    new_var_name = "Id_"+para_num
                elif source_code.startswith("mapping"):
                    idx1  = source_code.index("public")+6
                    para = source_code[idx1:]
                    if para not in tp_list:
                        tp_list.append(para)
                        if_static.append(1)
                    new_var_name = "Id_" + para
            else:
        ...
        if len(stack) > 0:
            ...
            if isReal(address) and global_params.USE_GLOBAL_BLOCKCHAIN:
                ...
                #print("Debug:new_var_name"+new_var_name)
                if new_var_name in path_conditions_and_vars:
                    new_var = path_conditions_and_vars[new_var_name]
                else:
                    new_var = BitVec(new_var_name, 256)
...
def detect_arithmetic_errors():
    ...
    pc1 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.OVERFLOW in arithmetic_error["type"] and arithmetic_error["validated"]]
    file1 = open("over.txt",'w')
    file1.write(str(pc1))
    pc2 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.UNDERFLOW in arithmetic_error["type"] and arithmetic_error["validated"]]
    file2 = open("under.txt",'w')
    file2.write(str(pc2))
    #still remain to solve
    '''pc3 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.DIVISION in arithmetic_error["type"] and arithmetic_error["validated"]]
    file3 = open("div.txt",'w')
    file3.write(str(pc3))
    pc4 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.MODULO in arithmetic_error["type"] and arithmetic_error["validated"]]
    file4 = open("mod.txt",'w')
    file4.write(str(pc4))
    pc5 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.WIDTH_CONVERSION in arithmetic_error["type"] and arithmetic_error["validated"]]
    file5 = open("wid.txt",'w')
    file5.write(str(pc5))
    pc6 = [arithmetic_error["constrains"] for arithmetic_error in arithmetic_errors if ErrorTypes.SIGNEDNESS in arithmetic_error["type"] and arithmetic_error["validated"]]
    file6 = open("sign.txt",'w')
    file6.write(str(pc6))'''
...
def main(contract, contract_sol, _source_map = None):
    ...
    detect_bugs()
    file_para = open("paras.txt","w")
    file_para.write(str(tp_list))
    file_static = open("if_static.txt","w")
    file_static.write(str(if_static))
    closing_message()
